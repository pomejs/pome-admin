...
var logger = require('pome-logger').getLogger('pome-admin', __filename);
...
MasterAgent.prototype.listen = function(port, cb) {
  ...
  this.server.httpServer.once('listening', function() {
...
var ipFilter = function(obj) {
  if(typeof this.whitelist === 'function') {
    ...
    self.whitelist(function(err, tmpList) {
      if(err) {
        logger.error('%j.(pome-admin whitelist).', err);
        return;
      }
      if(!Array.isArray(tmpList)) {
        logger.error('%j is not an array.(pome-admin whitelist).', tmpList);
        return;
      }
      if(!!obj && !!obj.ip && !!obj.id) {
        ...
        if(sock) {
          ...
          logger.error('%s is rejected(pome-admin whitelist).', obj.ip);
        }
      }
    });
  }
